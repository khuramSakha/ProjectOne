...
window.weatherButton = null;
window.fiananceButton = null;
window.covidButton = null;
window.wikiButton = null;
window.holidaysButton = null;
...
function addCountryListControl() {
  L.Control.CountryList = L.Control.extend({
    onAdd: function () {
      $select = $(
        '<select id="countryList" class="form-control leaflet-control leaflet-bar" validateCountry(this.value)></select>'
      );
      $select.append(
...
function getCountry({ countryName, lat, lng }) {
  ...
  $.getJSON("php/api.php", params, function (data, status) {
    // console.log(data);
    // alert('api.php')
    if (lat && lng) {
    ...
    if (data.opencage && data.rest) {
      ...
      window.weatherButton.enable();
      window.fiananceButton.enable();
      window.covidButton.enable();
      window.wikiButton.enable();
    }
  }).then(function () {
    $("#preloader").fadeOut();
    // map.spin(false);
...
function getCountryList() {
  $.getJSON("php/api.php", { get: "countryList" }, function (data, status) {
...
function init() {
  ...
  window.infoButton = L.easyButton(
    ...
    .addTo(map)
    .disable();
    window.weatherButton = L.easyButton(
      "fa-cloud",
      function () {
        $("#countryModal").modal("toggle");
      },
